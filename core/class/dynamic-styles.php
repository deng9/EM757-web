...
/**
* Foodmood Dynamic Styles
*
*
* @class        Foodmood_dynamic_styles
* @version      1.0
* @category     Class
* @author       WebGeniusLab
*/
...
class Foodmood_dynamic_styles{
	...
	public function register_script(){
		...
		$this->use_minify = Foodmood_Theme_Helper::get_option('use_minify') ? '.min' : '';
		// Register action
	...
	public function css_reg(){
	    /* Register CSS */
	    wp_enqueue_style('foodmood-default-style', get_bloginfo('stylesheet_url'));
	    // Flaticon register
		...
		wp_enqueue_style('foodmood-main', $this->gtdu . '/css/main'.$this->use_minify.'.css');
	}
	/* Register JS */
	public function js_reg(){

		wp_enqueue_script('foodmood-theme-addons', $this->gtdu . '/js/theme-addons'.$this->use_minify.'.js', array('jquery'), false, true);
		wp_enqueue_script('foodmood-theme', $this->gtdu . '/js/theme.js', array('jquery'), false, true);

	    wp_localize_script( 'foodmood-theme', 'wgl_core', array(
	        ...
	        'like' => esc_html__( 'Like', 'foodmood' ),
	        'unlike' => esc_html__( 'Unlike', 'foodmood' )
	        ) );
	...
	public function admin_css_reg(){
		...
		wp_enqueue_style('font-awesome', $this->gtdu . '/css/font-awesome.min.css');
		// Main admin styles
		wp_enqueue_style('foodmood-admin', $this->gtdu . '/core/admin/css/admin.css');
	...
	public function admin_js_reg(){
		...
		wp_localize_script('wp-color-picker', 'wpColorPickerL10n', array(
			'clear'            => esc_html__('Clear', 'foodmood'),
			'clearAriaLabel'   => esc_html__('Clear color', 'foodmood'),
			'defaultString'    => esc_html__('Default', 'foodmood'),
			'defaultAriaLabel' => esc_html__('Select default color', 'foodmood'),
			'pick'             => esc_html__('Select', 'foodmood'),
			'defaultLabel'     => esc_html__('Color value', 'foodmood'),
		));

		//Admin Js
		wp_enqueue_script('foodmood-admin', $this->gtdu . '/core/admin/js/admin.js');
		// If active Metabox IO
		if (class_exists( 'RWMB_Loader' )) {
			wp_enqueue_script('foodmood-metaboxes', $this->gtdu . '/core/admin/js/metaboxes.js');
		}
        ...
        wp_localize_script('foodmood-admin', 'wgl_verify', [
            ...
            'email' => esc_js($email),
            'message' => esc_js(esc_html__( 'Thank you, your license has been validated', 'foodmood' )),
	...
	public function get_elementor_css_cache_footer()
    {
        ...
        // footer option
        $footer_switch = Foodmood_Theme_Helper::get_option('footer_switch');
        ...
        if ($footer_switch) {
            $footer_content_type = Foodmood_Theme_Helper::options_compare('footer_content_type','mb_footer_switch','on');
            if (
				'pages' == $footer_content_type
				&& class_exists('\Elementor\Core\Files\CSS\Post')
            ) {

                $footer_page_select = Foodmood_Theme_Helper::options_compare('footer_page_select', 'mb_footer_switch', 'on');
	...
	public function add_style()
	{
		...
		/* Body Style
		/*-----------------------------------------------------------------------------------*/
		$page_colors_switch = Foodmood_Theme_Helper::options_compare('page_colors_switch','mb_page_colors_switch','custom');
		$use_gradient_switch = Foodmood_Theme_Helper::options_compare('use-gradient','mb_page_colors_switch','custom');
		if ($page_colors_switch == 'custom') {
			$theme_color = Foodmood_Theme_Helper::options_compare('page_theme_color','mb_page_colors_switch','custom');
			$theme_secondary_color = Foodmood_Theme_Helper::options_compare('page_theme_secondary_color','mb_page_colors_switch','custom');
			$theme_third_color = Foodmood_Theme_Helper::options_compare('page_theme_third_color','mb_page_colors_switch','custom');

			$bg_body = Foodmood_Theme_Helper::options_compare('body_background_color','mb_page_colors_switch','custom');
			// Go top color
			$scroll_up_bg_color = Foodmood_Theme_Helper::options_compare('scroll_up_bg_color','mb_page_colors_switch','custom');
			$scroll_up_arrow_color = Foodmood_Theme_Helper::options_compare('scroll_up_arrow_color','mb_page_colors_switch','custom');
			// Gradient colors
			$theme_gradient_from = Foodmood_Theme_Helper::options_compare('theme-gradient-from','mb_page_colors_switch','custom');
			$theme_gradient_to = Foodmood_Theme_Helper::options_compare('theme-gradient-to','mb_page_colors_switch','custom');
		} else {
			...
			$theme_third_color = esc_attr(Foodmood_Theme_Helper::get_option('theme-third-color'));

			$bg_body = esc_attr(Foodmood_Theme_Helper::get_option('body-background-color'));
			// Go top color
			$scroll_up_bg_color = Foodmood_Theme_Helper::get_option('scroll_up_bg_color');
			$scroll_up_arrow_color = Foodmood_Theme_Helper::get_option('scroll_up_arrow_color');
			// Gradient colors
			$theme_gradient = Foodmood_Theme_Helper::get_option('theme-gradient');
			$second_gradient = Foodmood_Theme_Helper::get_option('second-gradient');
		...
		for ($i = 1; $i <= 6; $i++) {
		    ${'header-h'.$i} = Foodmood_Theme_Helper::get_option('header-h'.$i);
		...
		/* Body Typography
		/*-----------------------------------------------------------------------------------*/
		$main_font = Foodmood_Theme_Helper::get_option('main-font');
		...
		/*-----------------------------------------------------------------------------------*/
		$additional_font = Foodmood_Theme_Helper::get_option('additional-font');
		$additional_font_family = $additional_line_height = $additional_font_size = $additional_font_weight = $additional_color = '';
		if (!empty($additional_font)) {
			$additional_font_family = esc_attr($additional_font['font-family']);
			$additional_font_size = esc_attr($additional_font['font-size']);
			$additional_font_weight = esc_attr($additional_font['font-weight']);
			$additional_color = esc_attr($additional_font['color']);
			$additional_line_height = esc_attr($additional_font['line-height']);
			$additional_line_height = !empty($additional_line_height) ? round(((int)$additional_line_height / (int)$additional_font_size), 3) : '';
		}
		...
		/*-----------------------------------------------------------------------------------*/
		$menu_font = Foodmood_Theme_Helper::get_option('menu-font');
		...
		$sub_menu_font = Foodmood_Theme_Helper::get_option('sub-menu-font');
		...
		$name_preset = Foodmood_Theme_Helper::header_preset_name();
		...
		$menu_color_top = Foodmood_Theme_Helper::get_option('header_top_color', $name_preset, $def_preset);
		if (!empty($menu_color_top['rgba'])) {
	        $menu_color_top = !empty($menu_color_top['rgba']) ? esc_attr($menu_color_top['rgba']) : '';
	    }

		$menu_color_middle = Foodmood_Theme_Helper::get_option('header_middle_color', $name_preset, $def_preset);
		if(!empty($menu_color_middle['rgba'])){
			$menu_color_middle = !empty($menu_color_middle['rgba']) ? esc_attr($menu_color_middle['rgba']) : '';
		}

		$menu_color_bottom = Foodmood_Theme_Helper::get_option('header_bottom_color', $name_preset, $def_preset);
		if(!empty($menu_color_bottom['rgba'])){
			$menu_color_bottom = !empty($menu_color_bottom['rgba']) ? esc_attr($menu_color_bottom['rgba']) : '';
		}

		$header_secondary_color_top = Foodmood_Theme_Helper::get_option('header_top_secondary_color', $name_preset, $def_preset);
		if (!empty($header_secondary_color_top['rgba'])) {
	        $header_secondary_color_top = !empty($header_secondary_color_top['rgba']) ? esc_attr($header_secondary_color_top['rgba']) : '';
	    }

		$header_secondary_color_middle = Foodmood_Theme_Helper::get_option('header_middle_secondary_color', $name_preset, $def_preset);
		if(!empty($header_secondary_color_middle['rgba'])){
			$header_secondary_color_middle = !empty($header_secondary_color_middle['rgba']) ? esc_attr($header_secondary_color_middle['rgba']) : '';
		}

		$header_secondary_color_bottom = Foodmood_Theme_Helper::get_option('header_bottom_secondary_color', $name_preset, $def_preset);
		if(!empty($header_secondary_color_bottom['rgba'])){
			$header_secondary_color_bottom = !empty($header_secondary_color_bottom['rgba']) ? esc_attr($header_secondary_color_bottom['rgba']) : '';
		}

		$header_sticky_secondary_color = Foodmood_Theme_Helper::get_option('header_sticky_secondary_color');

		// Set Queries width to apply mobile style
	    $sub_menu_color = Foodmood_Theme_Helper::get_option('sub_menu_color' ,$name_preset, $def_preset);
	    $sub_menu_bg = Foodmood_Theme_Helper::get_option('sub_menu_background' ,$name_preset, $def_preset);
	    $sub_menu_bg = $sub_menu_bg['rgba'];

	    $sub_menu_border = Foodmood_Theme_Helper::get_option('header_sub_menu_bottom_border', $name_preset, $def_preset);
		$sub_menu_border_height = Foodmood_Theme_Helper::get_option('header_sub_menu_border_height', $name_preset, $def_preset);
		$sub_menu_border_height = $sub_menu_border_height['height'];
		$sub_menu_border_color = Foodmood_Theme_Helper::get_option('header_sub_menu_bottom_border_color', $name_preset, $def_preset);
		...
		$mobile_sub_menu_bg = Foodmood_Theme_Helper::get_option('mobile_sub_menu_background');
		$mobile_sub_menu_bg = $mobile_sub_menu_bg['rgba'];

		$mobile_sub_menu_overlay = Foodmood_Theme_Helper::get_option('mobile_sub_menu_overlay');
		$mobile_sub_menu_overlay = $mobile_sub_menu_overlay['rgba'];

		$mobile_sub_menu_color = Foodmood_Theme_Helper::get_option('mobile_sub_menu_color');

		$hex_header_font_color = Foodmood_Theme_Helper::HexToRGB($header_font_color);
		$hex_theme_color = Foodmood_Theme_Helper::HexToRGB($theme_color);

		$hex_theme_content =  Foodmood_Theme_Helper::HexToRGB($content_color);

		// sticky header logo
	    $header_sticky_height = Foodmood_Theme_Helper::get_option('header_sticky_height');
	    ...
	    $footer_text_color = Foodmood_Theme_Helper::get_option('footer_text_color');
	    $footer_heading_color = Foodmood_Theme_Helper::get_option('footer_heading_color');

	    $copyright_text_color = Foodmood_Theme_Helper::options_compare('copyright_text_color','mb_copyright_switch','on');

		// Page Title Background Color
		$page_title_bg_color = Foodmood_Theme_Helper::get_option('page_title_bg_color');
		$hex_page_title_bg_color = Foodmood_Theme_Helper::HexToRGB($page_title_bg_color);
		...
		if(is_single()){
			if(function_exists('is_product') && is_product()){
				$layout_single = Foodmood_Theme_Helper::options_compare('shop_single_image_layout', 'mb_product_layout', 'custom');
				if($layout_single === 'with_background'){
					$layout_bg = Foodmood_Theme_Helper::get_option('shop_layout_with_background');
					if (class_exists( 'RWMB_Loader' ) && get_queried_object_id() !== 0) {
		...
		/* Side Panel Css
		/*-----------------------------------------------------------------------------------*/
		$side_panel_title = Foodmood_Theme_Helper::get_option('side_panel_title_color');
		...
		/*-----------------------------------------------------------------------------------*/
		$mobile_header = Foodmood_Theme_Helper::get_option('mobile_header');

		// Fetch mobile header height to apply it for mobile styles
		$header_mobile_height = Foodmood_Theme_Helper::get_option('header_mobile_height');
		...
		// Set Queries width to apply mobile style
		$header_queries = Foodmood_Theme_Helper::get_option('header_mobile_queris', $name_preset, $def_preset);
		$mobile_over_content = Foodmood_Theme_Helper::get_option('mobile_over_content');

		if ($mobile_header == '1') {
			$mobile_background = Foodmood_Theme_Helper::get_option('mobile_background');
			$mobile_color = Foodmood_Theme_Helper::get_option('mobile_color');
		...
		/*-----------------------------------------------------------------------------------*/
		$page_title_resp = Foodmood_Theme_Helper::get_option('page_title_resp_switch');
		$mb_cond_logic = false;
		...
		if ($page_title_resp == '1') {
			...
			$page_title_height = Foodmood_Theme_Helper::get_option('page_title_resp_height');
			$page_title_height = $page_title_height['height'];

			$page_title_queries = Foodmood_Theme_Helper::options_compare('page_title_resp_resolution', 'mb_page_title_resp_switch', $mb_cond_logic);

			$page_title_padding = Foodmood_Theme_Helper::options_compare('page_title_resp_padding', 'mb_page_title_resp_switch', $mb_cond_logic);

			if($mb_cond_logic == '1'){
				$page_title_height = rwmb_meta('mb_page_title_resp_height');
			}

			$page_title_font = Foodmood_Theme_Helper::options_compare('page_title_resp_font', 'mb_page_title_resp_switch', $mb_cond_logic);
			$page_title_breadcrumbs_font = Foodmood_Theme_Helper::options_compare('page_title_resp_breadcrumbs_font', 'mb_page_title_resp_switch', $mb_cond_logic);
			$page_title_breadcrumbs_switch = Foodmood_Theme_Helper::options_compare('page_title_resp_breadcrumbs_switch', 'mb_page_title_resp_switch', $mb_cond_logic);
		...
		$footer_switch = Foodmood_Theme_Helper::get_option('footer_switch');
		if ($footer_switch) {
			$footer_content_type = Foodmood_Theme_Helper::get_option('footer_content_type');
			if (class_exists( 'RWMB_Loader' ) && get_queried_object_id() !== 0) {
			...
			if($footer_content_type == 'pages'){
				$footer_page_id = Foodmood_Theme_Helper::options_compare('footer_page_select');
				if ( $footer_page_id ) {
		...
		wp_add_inline_style( 'foodmood-main', $css );
...
if(!function_exists('foodmood_dynamic_styles')){
    function foodmood_dynamic_styles() {
        return Foodmood_dynamic_styles::instance();
    }
}

foodmood_dynamic_styles()->register_script();
foodmood_dynamic_styles()->init_style();
