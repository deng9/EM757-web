...
/**
* Foodmood Theme Helper
*
*
* @class        Foodmood_Theme_Helper
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('Foodmood_Single_Post')) {

	class Foodmood_SinglePost{
		/**
	     * @var SinglePost
	     */
	    ...
	    public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null, $show_media = true ) {
			...
			$featured_image = Foodmood_Theme_Helper::options_compare('featured_image_type', 'mb_featured_image_conditional', 'custom');
			if ($featured_image == 'replace') {
				$featured_image_replace = Foodmood_Theme_Helper::options_compare('featured_image_replace', 'mb_featured_image_conditional', 'custom');
			}
	    	...
	    	if((bool) $media && (bool) $show_media){
	    		if($this->post_format === 'video' && $video_style == 'bg_video'){
	    			echo Foodmood_Theme_Helper::render_html($media);
	    	...
	    	if(is_single() && $video_style != 'bg_video'){
	    		...
	    		if($video_style == 'bg_video'){
			    	...
			    	if(has_post_thumbnail()) {
			    		...
			    		$output .= '<div class="wgl-video_popup button_align-center animation_simple with_image">';
			    		...
			    		if(isset($video_link[0])){
			    			...
			    			$output .= '<div class="videobox_link_wrapper">';
			    			$output .= '<div ' . $this->get_render_attribute_string( 'video-lightbox' ) . '>';
			    			$output .= '<i class="videobox_icon flaticon-play-button"></i>';
							$output .= '</div>';
							$output .= '</div>';
						}
			...
			<?php
           	...
           	?>
           		<div class="blog-post_quote-author">
           			<?php
           			echo Foodmood_Theme_Helper::render_html($autor_avatar);
	    ...
	    public function image_size_render($image_size){
	    	$arr = array();
	    	switch ($image_size) {
	    		case 'foodmood-840-720':
	    		$arr['width'] = '840';
	    		$arr['height'] = '720';
	    		break;
	    		case 'foodmood-740-560':
	    		$arr['width'] = '740';
	    		$arr['height'] = '560';
	    		break;
	    		case 'foodmood-700-550':
	    		$arr['width'] = '700';
	    		$arr['height'] = '550';
	    		break;
	    		case 'foodmood-440-440':
	    		$arr['width'] = '440';
	    		$arr['height'] = '440';
	    		break;
	    		case 'foodmood-420-300':
	    		$arr['width'] = '420';
	    		$arr['height'] = '300';
	    		break;
	    		case 'foodmood-180-180':
	    		$arr['width'] = '180';
	    		$arr['height'] = '180';
	    		break;
	    		default:
	    ...
	    public function render_post_meta ($args = false) {
            ...
            $date_flag = '<div class="date_post-flag">'.$svg.'</div>';
	    	...
	    	if($this->show_author_meta || $this->show_comments_meta || $this->show_date_meta || $this->show_category_meta){
		    	...
		    	<div class="meta-wrapper">
					...
					<?php if($this->show_date_meta) : ?>
						<div class="date_post"><?php echo esc_html(get_the_time(get_option( 'date_format' ))); echo Foodmood_Theme_Helper::render_html($date_flag); ?></div>
					<?php endif; ?>
					...
					<?php if($this->show_author_meta) : ?>
						...
						<?php echo esc_html__('by', 'foodmood'); ?>
					...
					<?php if($this->show_comments_meta) :
						...
						$comments_text = '' . $comments_num == 1 ? esc_html__('Comment', 'foodmood') : esc_html__('Comments', 'foodmood')  . '';
					?>
	    ...
	    public function render_excerpt ($symbol_count = false, $shortcode = false, $read_more = false, $read_more_text = false) {
			...
			if ((bool)Foodmood_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
	    ...
	    public function get_post_views($postID){
		    ...
		    if($count==''){
		    	...
		    	echo '<span class="counts_text">'. esc_html__(' View', 'foodmood'). '</span>';
		    }else{
				echo '<span class="counts">'. esc_html($count). '</span>';
		    	echo '<span class="counts_text">'. esc_html__(' Views', 'foodmood'). '</span>';
		    }
			echo '</div>';
	    ...
	    public function render_author_info () {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">'.esc_html__('About', 'foodmood').'</span>'.$user_first.' '.$user_last.'</h5>' : '';
		...
		public function get_render_attribute_string($element)
		{
			...
			return ' ' . Foodmood_Theme_Helper::render_html_attributes($this->render_attributes[$element]);
		}
