...
/**
* Foodmood Mega Menu Walker
*
*
* @class        Foodmood_Mega_Menu_Waker
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if( ! class_exists( 'Foodmood_Mega_Menu_Waker' )){

    class Foodmood_Mega_Menu_Waker extends Walker_Nav_Menu {
        ...
        */
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ){
            ...
            $menu_flag = '<span class="menu-item_flag">'.$svg.'</span>';

            $pimpa = '<span class="submenu_pimp"><svg viewBox="0 0 8 7.9" style="enable-background:new 0 0 8 7.9;" xml:space="preserve"><path d="M7.9,2.8C6.9-0.7,1.3-1,0.1,2.5c-0.4,1.1,1.2,1.7,1.8,0.8C2.5,2.2,5,2.1,5.5,3.4C6.2,5.2,3.2,6,2,5.4C1.4,5,0.5,5.8,1,6.4 C3.3,9.9,8.9,6.6,7.9,2.8z"/></svg></span>';

            $item_output .= $args->link_before . $pimpa . $menu_item . $menu_flag . '<span class="menu-item_plus"></span>' . $args->link_after;
            $item_output .= '</a>';
            ...
            if( $this->check_mega_menu_activate($depth) ){
                ...
                if( $this->check_mega_menu_categories($item) ){
                    ...
                    if((bool) $has_cat ){
                        ...
                        $output .= "<li class='menu-item is-active is-uploaded' data-id='".esc_attr($item->object_id)."' data-posts-count='".esc_attr($this->wgl_megamenu_posts_count)."'><a href='# class='mega-sub-cat'>". esc_html__( 'All', 'foodmood' ) ."</a></li>";
    ...
    /*-----------------------------------------------------------------------------------*/
    add_action( 'wp_nav_menu_item_custom_fields', 'foodmood_add_megamenu_fields', 30, 5 );
    function foodmood_add_megamenu_fields( $item_id, $item, $depth, $args, $id ){

        ?>

        <div class="clear"></div>
        <br />
        <strong><?php esc_html_e( 'Foodmood Mega Menu Settings:', 'foodmood' ); ?></strong> <em><?php esc_html_e( '(Only for Main Menu)', 'foodmood' ); ?></em>
        <div class="clear"></div>
        <div class='wgl_accordion_wrapper collapsible close widget_class'>
            <div class='wgl_accordion_heading'>
                <span class='wgl_accordion_title'><?php esc_html_e( 'WGL Mega Menu Settings', 'foodmood' ); ?></span>
            ...
            <div class="wgl-mega-menu-type">
                <p class="field-megamenu-type description description-wide">
                    <label for="edit-menu-item-megamenu-type-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Enable The Mega Menu?', 'foodmood' ); ?>
                        <select id="edit-menu-item-megamenu-type-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-type" name="menu-item-wgl-megamenu-type[<?php echo esc_attr( $item_id ) ?>]">
                            <option value=""><?php esc_attr_e( 'Disable', 'foodmood' ); ?></option>
                            <?php  if( $item->object == 'category' ){  ?>
                            <option value="sub-posts" <?php selected( $item->wgl_megamenu_type, 'sub-posts' ); ?>><?php esc_html_e( 'Posts - Vertical Sub-Categories Filter', 'foodmood' ); ?></option>
                            <option value="sub-hor-posts" <?php selected( $item->wgl_megamenu_type, 'sub-hor-posts' ); ?>><?php esc_html_e( 'Posts - Horizontal Sub-Categories Filter', 'foodmood' ); ?></option>
                            <?php } ?>
                            <option value="links" <?php selected( $item->wgl_megamenu_type, 'links' ); ?>><?php esc_html_e( 'Mega Menu Columns', 'foodmood' ); ?></option>
                        </select>
                    </label>
                </p>
                ...
                <?php if( $item->object == 'category' ){  ?>
                    <p class="field-megamenu-posts-count description description-wide">
                        <label for="edit-menu-item-megamenu-posts-count-<?php echo esc_attr( $item_id ) ?>">
                            <?php esc_html_e( 'Posts Count', 'foodmood' ); ?>
                ...
                <p class="field-megamenu-columns description description-wide">
                    <label for="edit-menu-item-megamenu-columns-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Number of Mega Menu Columns', 'foodmood' ); ?>
                ...
                <p class="field-megamenu-background-image description description-wide col-6">
                    <label for="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Image', 'foodmood' );?>
                        <input type="text" class="foodmood_media_url widefat code edit-menu-item-megamenu-background-image" name="menu-item-wgl-megamenu-background-image[<?php echo esc_attr( $item_id ) ?>]" id="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>" value="<?php echo esc_attr($item->wgl_megamenu_background_image); ?>">
                    </label>
                    <a href="#" class="button foodmood_media_upload"><?php esc_html_e('Upload', 'foodmood'); ?></a>
                </p>
                ...
                <p class="field-megamenu-background-repeat description description-wide col-6">
                    <label for="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Repeat', 'foodmood' ); ?>
                        <select id="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-repeat" name="menu-item-wgl-megamenu-background-repeat[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="no-repeat" <?php selected( $item->wgl_megamenu_background_repeat, 'no-repeat' ); ?>><?php esc_html_e( 'No Repeat', 'foodmood' ); ?></option>
                            <option value="repeat" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat' ); ?>><?php esc_html_e( 'Repeat', 'foodmood' ); ?></option>
                            <option value="repeat-x" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat-x' ); ?>><?php esc_html_e( 'Repeat X', 'foodmood' ); ?></option>
                            <option value="repeat-y" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat-y' ); ?>><?php esc_html_e( 'Repeat Y', 'foodmood' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="field-megamenu-background-pos-x description description-wide col-6">
                    <label for="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position X', 'foodmood' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-x" name="menu-item-wgl-megamenu-background-pos-x[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="right" <?php selected( $item->wgl_megamenu_background_pos_x, 'right' ); ?>><?php esc_html_e( 'Right', 'foodmood' ); ?></option>
                            <option value="center" <?php selected( $item->wgl_megamenu_background_pos_x, 'center' ); ?>><?php esc_html_e( 'Center', 'foodmood' ); ?></option>
                            <option value="left" <?php selected( $item->wgl_megamenu_background_pos_x, 'left' ); ?>><?php esc_html_e( 'Left', 'foodmood' ); ?></option>
                        </select>
                    </label>
                </p>
                ...
                <p class="field-megamenu-background-pos-y description description-wide col-6">
                    <label for="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position Y', 'foodmood' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-y" name="menu-item-wgl-megamenu-background-pos-y[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="top" <?php selected( $item->wgl_megamenu_background_pos_y, 'top' ); ?>><?php esc_html_e( 'Top', 'foodmood' ); ?></option>
                            <option value="center" <?php selected( $item->wgl_megamenu_background_pos_y, 'center' ); ?>><?php esc_html_e( 'Center', 'foodmood' ); ?></option>
                            <option value="bottom" <?php selected( $item->wgl_megamenu_background_pos_y, 'bottom' ); ?>><?php esc_html_e( 'Bottom', 'foodmood' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="field-megamenu-min-height description description-wide col-6">
                    <label for="edit-menu-item-megamenu-min-height-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Min Height', 'foodmood' );
                        ?>
                ...
                <p class="field-megamenu-width description description-wide col-6">
                    <label for="edit-menu-item-megamenu-width-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Max Width', 'foodmood' );
                        ?>
                 ...
                 <p class="field-megamenu-padding-left description description-wide col-6">
                    <label for="edit-menu-item-megamenu-padding-left-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Left', 'foodmood' );
                        ?>
                ...
                <p class="field-megamenu-padding-right description description-wide col-6">
                    <label for="edit-menu-item-megamenu-padding-right-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Right', 'foodmood' );
                        ?>
                ...
                <p class="field-megamenu-hide-headings description description-wide">
                    <label for="edit-menu-item-megamenu-hide-headings-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Hide Mega Menu headings?', 'foodmood' );?>
    ...
    add_action('wp_update_nav_menu_item', 'foodmood_custom_nav_update', 10, 3);
    function foodmood_custom_nav_update( $menu_id, $item_id, $args ){
    ...
    add_filter( 'wp_edit_nav_menu_walker', 'foodmood_custom_nav_edit_walker',10,2 );
    function foodmood_custom_nav_edit_walker($walker,$menu_id){
        return 'Foodmood_Mega_Menu_Edit_Walker';
    }
     ...
     * @see Walker_Nav_Menu
     */
    class Foodmood_Mega_Menu_Edit_Walker extends Walker_Nav_Menu {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)', 'foodmood' ), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)', 'foodmood'), $item->title );
            }
                ...
                <div class="menu-item-bar">
                    <div class="menu-item-handle">
                        <span class="item-title">
                            <span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo !empty($submenu_text) ? $submenu_text : ''; ?>><?php esc_html_e( 'sub item', 'foodmood' ); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up" aria-label="<?php esc_attr_e( 'Move up', 'foodmood' ) ?>">&#8593;</a>
                                |
                                <a href="<?php
                                    echo esc_url(wp_nonce_url(
                                        add_query_arg(
                                ...
                                ?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'foodmood'  ) ?>">&#8595;</a>
                            </span>
                            ...
                            ?>" aria-label="<?php esc_attr_e( 'Edit menu item', 'foodmood'  ); ?>"><span class="screen-reader-text"><?php esc_html_e( 'Edit', 'foodmood' ); ?></span></a>
                        </span>
                    </div>
                </div>
                ...
                <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                    <?php if ( 'custom' == $item->type ) : ?>
                        <p class="field-url description description-wide">
                            <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                                <?php esc_html_e( 'URL', 'foodmood' ); ?><br />
                    ...
                    <p class="description description-wide">
                        <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Navigation Label', 'foodmood' ); ?><br />
                            <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                        </label>
                    </p>
                    <p class="field-title-attribute field-attr-title description description-wide">
                        <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Title Attribute', 'foodmood' ); ?><br />
                    ...
                    <p class="field-link-target description">
                        <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                            ...
                            <?php esc_html_e( 'Open link in a new tab', 'foodmood' ); ?>
                        </label>
                    </p>
                    <p class="field-css-classes description description-thin">
                        <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'CSS Classes (optional)', 'foodmood' ); ?><br />
                            <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                        </label>
                    </p>
                    <p class="field-xfn description description-thin">
                        <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Link Relationship (XFN)', 'foodmood' ); ?><br />
                            <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                        </label>
                    </p>
                    <p class="field-description description description-wide">
                        <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Description', 'foodmood' ); ?><br />
                            <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                            <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'foodmood'); ?></span>
                        </label>
                    </p>
                    <?php
                        /*-----------------------------------------------------------------------------------*/
                    ...
                    <fieldset class="field-move hide-if-no-js description description-wide">
                        <span class="field-move-visual-label" aria-hidden="true"><?php esc_html_e( 'Move', 'foodmood' ); ?></span>
                        <button type="button" class="button-link menus-move menus-move-up" data-dir="up"><?php esc_html_e( 'Up one', 'foodmood' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-down" data-dir="down"><?php esc_html_e( 'Down one', 'foodmood' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-left" data-dir="left"></button>
                        <button type="button" class="button-link menus-move menus-move-right" data-dir="right"></button>
                        <button type="button" class="button-link menus-move menus-move-top" data-dir="top"><?php esc_html_e( 'To the top', 'foodmood' ); ?></button>
                    </fieldset>
                    ...
                    <div class="menu-item-actions description-wide submitbox">
                        <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                            <p class="link-to-original">
                                ...
                                printf( wp_kses( __('Original: %s', 'foodmood'), $allowed_html ), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                            </p>
                        <?php endif; ?>
                        ...
                        )); ?>"><?php esc_html_e( 'Remove', 'foodmood' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                            ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'foodmood'); ?></a>
                    </div>
