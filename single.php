...
$sb = Foodmood_Theme_Helper::render_sidebars('single');


$single_type = Foodmood_Theme_Helper::get_option('single_type_layout'); 
...
<div class="wgl-container<?php echo apply_filters('foodmood_container_class', $container_class); ?>">
        <div class="row<?php echo apply_filters('foodmood_row_class', $row_class); ?>">
			<div id='main-content' class="wgl_col-<?php echo apply_filters('foodmood_column_class', $column); ?>">
				...
				if ($nextPost || $previousPost):
					?>
					<div class="foodmood-post-navigation">
						<?php
				...
				$show_post_related = Foodmood_Theme_Helper::get_option('single_related_posts');
				...
				if ( (bool)$show_post_related && class_exists('Foodmood_Core') && class_exists('\Elementor\Plugin')) : ?>
					<?php
					...
					$mb_blog_carousel_r 	  = Foodmood_Theme_Helper::options_compare('blog_carousel_r', 'mb_blog_show_r', 'custom');
					$mb_blog_title_r 	  	  = Foodmood_Theme_Helper::options_compare('blog_title_r', 'mb_blog_show_r', 'custom');

					$cats = Foodmood_Theme_Helper::get_option('blog_cat_r');
					if(!empty($cats)){
					...
					$mb_blog_column_r 	  	  = Foodmood_Theme_Helper::options_compare('blog_column_r', 'mb_blog_show_r', 'custom');
					$mb_blog_number_r 	      = Foodmood_Theme_Helper::options_compare('blog_number_r', 'mb_blog_show_r', 'custom');
					...
					<?php
						...
						echo '<div class="foodmood_module_title"><h3>'.(!empty($mb_blog_title_r) ? esc_html($mb_blog_title_r) : esc_html__('Recent Posts', 'foodmood')) .' </h3></div>';
						...
						$atts['read_more_text'] 		= '';
						$atts['heading_tag'] 			= 'h4';
						$atts['content_letter_count'] 	= 90;
						$atts['crop_square_img'] 		= 1;
						$atts['items_load'] 			= 4;
						$atts['name_load_more'] 		= esc_html__('Load More','foodmood');
        				...
        				echo Foodmood_Theme_Helper::render_html($related_items->render($atts));
						?>
					</div>
					<?php
				endif;
