...
use Foodmood_Theme_Helper as Foodmood;

if (!class_exists('Foodmood_breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
     * @package foodmood\templates
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     * @version 1.1.2
     */
	class Foodmood_breadcrumbs
	{
        ...
        {
			$this->text_home     = esc_html__('Home', 'foodmood'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'foodmood'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'foodmood'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'foodmood'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'foodmood'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'foodmood'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'foodmood'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'foodmood'); // text 'Comment Page N'
        }
        ...
        public function __construct ()
        {
			...
			if (is_home() || is_front_page()) {
				...
				if (
                    ...
                    && (is_product() || is_product_category() || is_product_tag() || is_cart() || is_checkout())
                ) {
					$args = [];
					$args['delimiter'] = $sep;
					...
					$out .= foodmood_woocommerce_breadcrumb($args);
				} elseif (
                    class_exists('WooCommerce')
                    && is_woocommerce()
                    && !is_product()
                    && ($shop_id = wc_get_page_id( 'shop' )) > 0
                ) {
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(Foodmood::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep
                            . sprintf($link, $archive_link, esc_html__('Portfolio', 'foodmood'))
                            . $sep
                            . $before
				...
				// Custom Post Type
                    ...
                    && is_product()
                    && function_exists('foodmood_woocommerce_breadcrumb')
                ) {
					$args = [];
					$args['delimiter'] = $sep;
					if ($show_home_link) $out .= $sep;
		    ...
		    echo Foodmood::render_html($out);
    	}
    }

    new Foodmood_breadcrumbs();
}
