...
*
*
* @class		Foodmood_get_page_title
* @version		1.0
* @category		Class
* @author		WebGeniusLab
*/

if (!class_exists('Foodmood_get_page_title')) {
	class Foodmood_get_page_title{
		...
		public function init() {
	   	...
	   	public function page_title_render_html() {
			if ($this->page_title_switch == 'on') {
				$page_title_font = Foodmood_Theme_Helper::options_compare('page_title_font', 'mb_page_title_switch', 'on');
				$page_title_breadcrumbs_font = Foodmood_Theme_Helper::options_compare('page_title_breadcrumbs_font', 'mb_page_title_switch', 'on');
				$page_title_breadcrumbs_switch = Foodmood_Theme_Helper::options_compare('page_title_breadcrumbs_switch', 'mb_page_title_switch', 'on');

				$page_title_parallax = Foodmood_Theme_Helper::options_compare('page_title_parallax', 'mb_page_title_switch', 'on');
				$page_title_parallax_speed = apply_filters("pagetitle_parallax_speed", Foodmood_Theme_Helper::options_compare('page_title_parallax_speed', 'mb_page_title_switch', 'on'));

				$page_title_parallax_mouse = Foodmood_Theme_Helper::options_compare('page_title_parallax_mouse', 'mb_page_title_switch', 'on');
				$page_title_parallax_speed_mouse = apply_filters("pagetitle_parallax_speed_mouse", Foodmood_Theme_Helper::options_compare('page_title_parallax_speed_mouse', 'mb_page_title_switch', 'on'));
				...
				$foodmood_page_title = $this->foodmood_page_title();
				...
				if ($this->page_title_switch == 'on') {
					...
					$output = "<div class='page-header". (!empty($classes) ? esc_attr($classes) : '') . $page_title_parallax_class . "'".( !empty($styles) ? ' style="'.esc_attr($styles).'"' : '')." ".$page_title_parallax_data.">";
						...
						if ((bool)$page_title_parallax_mouse) {
							...
							$bg_render_mouse = Foodmood_Theme_Helper::bg_render('page_title_mouse','mb_page_title_switch','on');
							...
							$output .= "<div class='wgl-container'>";
								$output .= "<div class='page-header_content'>";
									...
									if(function_exists('is_product') && is_product()){
										if(function_exists('foodmood_woocommerce_prev_next')){
											$output .= foodmood_woocommerce_prev_next();
										}
									}
		...
		public function foodmood_page_title() {
			$title = '';
			if (is_home() || is_front_page()) {
				...
				$title = single_term_title("", false).esc_html__( ' Tag', 'foodmood' );
			} elseif ( is_date() ) {
				$title = get_the_time('F Y');
			} elseif( is_author() ) {
				$title = esc_html__( 'Author:', 'foodmood' ) .' '. get_the_author();
			} elseif ( is_search() ) {
				$title = esc_html__( 'Search', 'foodmood' );
			} elseif ( is_404() ) {
				$title = (bool)Foodmood_Theme_Helper::get_option('404_custom_title_switch') ? Foodmood_Theme_Helper::get_option('404_page_title_text') : esc_html__( 'Error Page', 'foodmood' );
			} elseif ( is_archive() ) {
				...
				$portfolio_title_conditional = Foodmood_Theme_Helper::get_option('portfolio_title_conditional') == '1' ? 'on' : 'off';
	            $portfolio_title_text = !empty(Foodmood_Theme_Helper::get_option('portfolio_single_page_title_text')) ? Foodmood_Theme_Helper::get_option('portfolio_single_page_title_text') : '';

				$title = $portfolio_title_conditional == 'on' ? esc_html($portfolio_title_text) : esc_html(get_the_title());
				$title = apply_filters('foodmood_page_title_portfolio_text', $title);

	        } elseif ( is_singular('team') ) {

				$team_title_conditional = Foodmood_Theme_Helper::get_option('team_title_conditional') == '1' ? 'on' : 'off';
	            $team_title_text = !empty(Foodmood_Theme_Helper::get_option('team_single_page_title_text')) ? Foodmood_Theme_Helper::get_option('team_single_page_title_text') : '';

				$title = $team_title_conditional == 'on' ? esc_html($team_title_text) : esc_html(get_the_title());
				$title = apply_filters('foodmood_page_title_team_text', $title);

	        } elseif ( function_exists('is_product') && ( is_product() ) ) {
	        	$shop_title_conditional = Foodmood_Theme_Helper::get_option('shop_title_conditional') == '1' ? 'on' : 'off';
	            $shop_title_text = !empty(Foodmood_Theme_Helper::get_option('shop_single_page_title_text')) ? Foodmood_Theme_Helper::get_option('shop_single_page_title_text') : '';

				$title = $shop_title_conditional == 'on' ? esc_html($shop_title_text) : esc_html(get_the_title());
				$title = apply_filters('foodmood_page_title_shop_text', $title);
	        }
	        else {
	            global $post;
	            ...
	            if (!empty($post)) {
	                ...
	                $blog_title_conditional = Foodmood_Theme_Helper::get_option('blog_title_conditional') == '1' ? 'on' : 'off';
	                $blog_title_text = !empty(Foodmood_Theme_Helper::get_option('post_single_page_title_text')) ? Foodmood_Theme_Helper::get_option('post_single_page_title_text') : '';

	                if ($posttype == 'post') {
						$title = $blog_title_conditional == 'on' ? esc_html($blog_title_text) : esc_html(get_the_title($id));
						$title = apply_filters( 'foodmood_page_title_blog_text', $title );
	                } else {
	                ...
	                $title = esc_html__('No Posts','foodmood');
	            }
	        ...
	        if ($this->mb_page_title_switch == 'on') {
	        	...
	        	if(!empty($custom_title_switch)){
	        		...
	        		$title = apply_filters( 'foodmood_page_title_custom_text', $title );
	        	}
	        }
	   	...
	   	public function page_title_classes() {
			...
			if ( is_singular('portfolio') || function_exists('is_product') && is_product() ) { // Portfolio single, Shop single have individual options for fine customization
				...
				$page_title_align = Foodmood_Theme_Helper::get_option($post_type.'_single_title_align');
				$breadcrumbs_align = Foodmood_Theme_Helper::get_option($post_type.'_single_breadcrumbs_align');
				$breadcrumbs_block = Foodmood_Theme_Helper::get_option($post_type.'_single_breadcrumbs_block_switch');
				...
				$page_title_align = Foodmood_Theme_Helper::options_compare( 'page_title_align', 'mb_page_title_switch', 'on' );
				$breadcrumbs_align = Foodmood_Theme_Helper::options_compare( 'page_title_breadcrumbs_align', 'mb_page_title_switch', 'on' );
				$breadcrumbs_block = Foodmood_Theme_Helper::get_option('page_title_breadcrumbs_block_switch');
			}
	   	...
	   	public function page_title_styles() {
			$page_title_bg_switch = Foodmood_Theme_Helper::options_compare( 'page_title_bg_switch', 'mb_page_title_switch', 'on' );
			$page_title_margin = Foodmood_Theme_Helper::options_compare( 'page_title_margin', 'mb_page_title_switch', 'on' );
			$page_title_padding = Foodmood_Theme_Helper::options_compare( 'page_title_padding', 'mb_page_title_switch', 'on' );
			$page_title_border_top_switch = false;
			$page_title_border_color = '';

			if ($this->page_title_switch == 'on') {
				$page_title_bg_color = Foodmood_Theme_Helper::get_option('page_title_bg_image')['background-color'];
				$page_title_height = Foodmood_Theme_Helper::get_option('page_title_height')['height'];
			}
			...
			if ( is_singular('portfolio') || function_exists('is_product') && is_product() || is_404() ) {
				...
				$page_title_bg_switch = Foodmood_Theme_Helper::get_option($post_type.'_title_bg_switch');
				$page_title_bg_color = Foodmood_Theme_Helper::get_option($post_type.'_page_title_bg_image')['background-color'];
				$page_title_margin = Foodmood_Theme_Helper::get_option($post_type.'_page_title_margin');
				$page_title_padding = Foodmood_Theme_Helper::get_option($post_type.'_page_title_padding');
				if ( class_exists('RWMB_Loader') && ($this->id !== 0) && (rwmb_meta('mb_page_title_switch') == 'on') ) {
				...
				if ($post_type == 'shop_single') {
					$page_title_border_top_switch = Foodmood_Theme_Helper::get_option($post_type.'_page_title_border_switch');
					$page_title_border_color = Foodmood_Theme_Helper::get_option($post_type.'_page_title_border_color')['rgba'];
					if ( class_exists('RWMB_Loader') && ($this->id !== 0) && (rwmb_meta('mb_page_title_switch') == 'on') ) {
						$page_title_border_top_switch = rwmb_meta('mb_page_title_border_switch');
						$page_title_border_color = rwmb_meta('mb_page_title_border_color');
					}
			...
			if ( is_404() ) {
				switch ( (bool)$page_title_bg_switch ) {
					case true:
						$style .= !empty(Foodmood_Theme_Helper::bg_render('404_page_title')) ? Foodmood_Theme_Helper::bg_render('404_page_title') : Foodmood_Theme_Helper::bg_render('page_title');
						break;
					default: break;
				}
			} elseif ( (bool)$shop_title && !empty(Foodmood_Theme_Helper::bg_render('shop_'.$shop_title.'_page_title')) ) {
				$style .= function_exists('is_product') && !is_product() ? Foodmood_Theme_Helper::bg_render('shop_'.$shop_title.'_page_title') : ((bool)$page_title_bg_switch ? Foodmood_Theme_Helper::bg_render('shop_single_page_title') : '');
			} elseif ( (bool)$cpt_title && (bool)$page_title_bg_switch && !empty(Foodmood_Theme_Helper::bg_render($cpt_type_title.'_'.$cpt_title.'_page_title')) ) {
				$style .= Foodmood_Theme_Helper::bg_render($cpt_type_title.'_'.$cpt_title.'_page_title');
			} else {
				$style .= (bool)$page_title_bg_switch ? Foodmood_Theme_Helper::bg_render('page_title','mb_page_title_switch','on') : '';
			}
