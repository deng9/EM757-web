...
if (!class_exists('Foodmood_footer_area')) {
	/**
	 * Footer area
	 *
	 *
	 * @class 		Foodmood_footer_area
	 * @version		1.0
	 * @category	Class
	 * @author 		WebGeniusLab
	 */
    ...
    class Foodmood_footer_area {
		...
		*/
    	...
    	function __construct () {
            $footer_options = apply_filters( 'foodmood_footer_enable', true);
            extract($footer_options);

            $footer_bg_color = Foodmood_Theme_Helper::get_option('footer_bg_color');
            $style = '';

            // Upper-level variables
            $this->id = get_queried_object_id();
            $this->content_type = Foodmood_Theme_Helper::options_compare('footer_content_type', 'mb_footer_switch', 'on');
            $this->footer_full_width = Foodmood_Theme_Helper::get_option('footer_full_width');
            ...
            if (
                ...
                $style .= Foodmood_Theme_Helper::bg_render('footer', 'mb_footer_switch', 'on');
    	...
    	private function get_wave_html(){
    		$wave_switch = Foodmood_Theme_Helper::options_compare('footer_add_wave','mb_footer_switch','on');
    		if((bool) $wave_switch){

    			$wave_height = Foodmood_Theme_Helper::get_option('footer_wave_height');
    			...
    			echo "<div class='foodmood_wave_footer' style='height:".(int) $wave_height."px;'>";
    	...
    	private function get_footer_vars($optn_1 = null){
    		...
    		//Get options
			$footer_spacing = Foodmood_Theme_Helper::options_compare('footer_spacing','mb_footer_switch','on');
			$footer_border = Foodmood_Theme_Helper::options_compare('footer_add_border','mb_footer_switch','on');
			$footer_border_color = Foodmood_Theme_Helper::options_compare('footer_border_color','mb_footer_switch','on');

	        // Only for widgets in footer
	        if ($optn_1 == 'widgets') {
				$footer_options['widget_columns'] = Foodmood_Theme_Helper::get_option('widget_columns');
		        $footer_options['widget_columns_2'] = Foodmood_Theme_Helper::get_option('widget_columns_2');
		        $footer_options['widget_columns_3'] = Foodmood_Theme_Helper::get_option('widget_columns_3');
		        $footer_align = Foodmood_Theme_Helper::get_option('footer_align');
    	...
    	private function copyright_spacing(){
	        //Get options
    		$copyright_spacing = Foodmood_Theme_Helper::options_compare('copyright_spacing','mb_copyright_switch','on');
    	...
    	private function copyright_style(){
			$bg_color = Foodmood_Theme_Helper::options_compare('copyright_bg_color','mb_copyright_switch','on');
    	...
    	private function copyright_html() {
	        $editor = Foodmood_Theme_Helper::get_option('copyright_editor');

	        if ($this->mb_copyright_switch == 'on') {
	        	$editor = rwmb_meta('mb_copyright_editor');
	        }
	        ?>
    		<div class='copyright'<?php echo Foodmood_Theme_Helper::render_html($this->copyright_style()); ?> >
                <?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
                	<div class='row' <?php echo Foodmood_Theme_Helper::render_html($this->copyright_spacing());?> >
